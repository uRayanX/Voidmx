 ...
 * Fetches a YouTube playlist via Invidious (same as album but returns items format).
 */
const INVIDIOUS = [
...
export async function onRequest(context) {
  ...
  for (const base of INVIDIOUS) {
    try {
      ...
      const items = (data.videos || []).filter(v => v.videoId).map(v => ({
      ...
      return new Response(JSON.stringify({
        id, title: data.title, author: data.author, items,
        cover: data.playlistThumbnail || '',
      }), { headers: CORS_HEADERS });
    } catch {}
  }
  return new Response(JSON.stringify({ id, title: '', items: [] }), { headers: CORS_HEADERS });
}
